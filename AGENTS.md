...
- Backend: Flask (`backend/app.py`, launcher at `app.py`).
- Frontend: single HTML (`templates/index.html`).
- Render runner: shell wrapper (`scripts/render_job.sh`) + Blender Python (`backend/turbo_optimize.py`).
...
## Key Files
- `backend/app.py`: Flask API, worker loop, job lifecycle, config endpoints.
- `templates/index.html`: UI and client JS.
- `scripts/render_job.sh`: shell wrapper (calls Blender).
- `backend/turbo_optimize.py`: Blender Python hooks and render settings.
- `config.json`: runtime config + `TURBO_SETTINGS`.

## Common Tasks
- UI changes: update `templates/index.html` only (no build step).
- Backend changes: update `backend/app.py` and restart Flask.
- Render pipeline changes: update `scripts/render_job.sh` and/or `backend/turbo_optimize.py`.

## Testing
- Quick sanity: `python3 -m py_compile app.py backend/*.py`
