...
export RENDER_PIPELINE="${RENDER_PIPELINE:-NORMAL}"
export RENDER_OUTPUT_DIR="${RENDER_OUTPUT_DIR:-$JOB_DIR/renders}"
export RENDER_FRAME="${RENDER_FRAME:-1}"
mkdir -p "$RENDER_OUTPUT_DIR"

echo "[render_job] blender=$BLENDER_BIN mode=${RENDER_MODE} pipeline=${RENDER_PIPELINE} hiprt=${USE_HIPRT} gpu_name=${RENDER_GPU_NAME} frame=${RENDER_FRAME} blend=$BLEND_PATH output=$RENDER_OUTPUT_DIR"
